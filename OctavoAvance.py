# FUNCIÓN PARA DETERMINAR EL INTERVALO DE LA JUNTA
def mes(meses, mesi, mesf, diai, diaf):
    ...
    for cont in meses:  # (cont es el mes actual)
        ...
        for info in cont:  # info es la fecha (día/mes)
            # con este if me aseguro que se cierre en el dia final cuando mesi = mesf
            if mesf == mesi and info[0] == diaf + 1:
                break
            # aquí hago que meta todos los días del primer mes
            if info[1] == mesi and info[0] >= diai:
                inter.append(info)
            # aquí hago meta lo demás del intervalo
            if mesf >= info[1] > mesi:
                inter.append(info)
                # corta el día final del mes final
                if info[0] == diaf and info[1] == mesf:
                    break
    return inter


# FUNCIÓN PARA DETERMINAR MÍNIMO MÁXIMO DE UN MES
def max_y_min(intervalo, mesi, mesf):
    mlista = []
    # ESTOS FOR AGREGAN TODOS LOS DÍAS A UNA LISTA DIVIDIDO POR MES
    # LUEGO SE HACE OTRA LISTA QUE BUSCA LOS DÍAS MAX Y MIN / MES
...
# FUNCIÓN PARA QUITAR UN DÍA EN ESPECÍFICO "N" CANTIDAD DE VECES
def quitar_intervalo(intervalo, mlista, mesi, mesf):
    ...
    while cond == 1:
        ...
        # con el for y el if estoy comprobando que si este dentro del rango posible
        for c in mlista:
            if (c[1] >= diaq >= c[0]) and (mesq == c[2]) and (mesi <= mesq <= mesf):
                pase = True
        if not pase:
            print("Esto no se puede, inténtelo de nuevo")
        # una vez comprobado se pasa por un if para saber que día es específicamente, contando el mes
        for i in inter:
...
# FUNCIÓN QUE QUITA ASISTENCIA A DIAS ESPECÍFICOS
def asistencia(intervalo, mlista, mesi, mesf):
    print("\n")
    # Aquí estoy asignando un valor a cantusuarios en el intervalo, para poder restar a los días que no van a ir
    cantusuarios = int(input("Digite la cantidad de usuarios que asistirán a la reunion: "))
    for i in intervalo:
        i[2] = cantusuarios
    print("\n")

    # En esta función voy a preguntar por cada usuario si es que no pueden ir un día
    for i in range(cantusuarios):
        # cont, mest y diat son variables que uso para asegurarme de que un usuario no quite el mismo día dos veces
        cont = i
        mest = 0
        diat = 0
        # asis lo defino como 1 para que pase, pero su valor se modifica en el while
        asis = 1
        # pase se usa para asegurar que los valores dados estén dentro del intervalo
        pase = False
        while asis == 1:
            ...
            # aquí me aseguro de que un usuario no quite el mismo día dos veces
            if cont > i and (mest == quitar_asistencia_mes and diat == quitar_asistencia_dia):
                ...
                # Este for asegura que los valores dados estén dentro del intervalo y cambia el valor de asistentes/día
                for c in mlista:
            ...
            cont += 1
            """
            debo de hacer que funcione que el usuario no digite el mismo valor dos veces
            todo pq no funciona cuando pones un día distinto y vuelves a poner el mismo, (puedo hacerlo lista)
            """
            # si no está dentro del rango del intervalo se repite
            if not pase:
                print("Esto no se puede, inténtelo de nuevo")
        print("\n")


# FUNCIÓN QUE HACE LA SUMA DE ASISTENCIAS
...
# en este while estoy preguntando a todos los usuarios si pueden ir
                while acm < usuarios:
                    ...
                    # aquí esta iterando la cantidad de usuarios que si pueden asistir a la reunion con un IF
                    , luego lo imprime
                    if asis == 1:
                        info[2] += asis
                        print(info[2])
...
# FUNCIÓN PARA CONFIRMAR QUE UN VALOR SEA INT
def es_num(cadena):
    # Intenta leer cadena como un entero, si no puede regresa False
    try:
        int(cadena)
        return True
    except ValueError:
...
def main():
    ...
    # Con este while me aseguro de *mesi* este dentro del rango de un mes Y que sea valor NUMÉRICO
    ...
    # Con estos while me aseguro que sea NUMÉRICO, que esté en RANGO
    diai = (input('Digite el día inicial: '))  # 10
    ...
    print("\n")
    mesf = (input('Digite el mes final: '))  # 6
    # Con estos whiles me aseguro, que sea numérico, que sea igual o mayor al final y que esté en rango
    ...
    # Con estos while me aseguro que sea NUMÉRICO, que esté en RANGO,
    # Asegura que si esta en el mismo mes, tiene que ser MAYOR a diai
    diaf = (input('Digite el día final: '))  # 12
    ...
    # Asegura que si esta en un mes de 30 o 28 días, que diaf no sea MAYOR a ello
